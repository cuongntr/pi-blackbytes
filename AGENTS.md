...
- `sub_agents.<name>.timeoutMs` (per-agent timeout, 1..3600000 ms; YAML uses `timeout_ms`. Builtin defaults: explore=600000, librarian=900000, oracle=1200000, general=1800000)
...
Declarations carry optional `systemPromptByFamily` (per-model-family prompt overrides) and `routing` (`SubAgentRoutingMetadata`) fields. All four builtins (explore, oracle, librarian, general) define GPT-optimized prompt variants selected via `resolveSystemPromptBody()` in `src/sub-agents/prompt-builder.ts` when the configured nested model classifies as GPT family. Oracle handles difficult code, plan, and architecture review while remaining read-only; General owns implementation, final-diff self-review, fixes, and verification. Routing metadata drives the Bytes overlay routing matrix and `/blackbytes-status`; prompt contract regression tests guard each role's output contract.
