...
A pure diagnostics summary builder (`src/sub-agents/diagnostics-summary.ts`) aggregates per-agent configuration, recent failures grouped by kind, YAML loader warnings, and delegation success rate into a testable data structure. A lazy nested Pi availability check (`src/sub-agents/pi-availability.ts`) reports whether the `pi` CLI can be spawned, using a cached probe with a 2-second timeout. Both feed the Sub-Agent Diagnostics section in `/blackbytes-status`.
