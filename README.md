...
| `sub_agents.<name>.artifactCapture` | `boolean` | Opt-in persistence of large redacted sub-agent outputs to `$PI_AGENT_DIR/blackbytes/artifacts/sub-agents/<YYYY-MM-DD>/<agent>-<HHmmssSSS>.md` (512 KiB cap per file, 7-day retention). Defaults to `false`. The artifact directory, total count, and most recent artifact surface under `/blackbytes-status` → Sub-Agent Diagnostics. |
